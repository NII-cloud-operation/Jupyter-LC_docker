...
Jupyter Notebook server which bundles a set of tools for *Literate Computing for Reproducible Infrastructure*.  This bundle shows how to implement robust and reliable operational procedure rather than non-linier explorative one.
...
* Adding good perspective and awareness; markdown’s hierarchy is collapsible as document according to your focus.  However, embedded cells underneath are represented as dots and run through with a click as a routine procedure.
* Enabling affluent communication via notebooks; collaborative annotation powered by Scrapbox.
...
- `-e lc_wrapper_force=on` - Force summarizing mode
